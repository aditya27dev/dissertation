...
df_test = pd.read_excel('./hasoc/hasoc_2020_en_test_new.xlsx')

def preprocessing():

    df = pd.read_excel('./hasoc/hasoc_2020_en_test_new.xlsx')

    df['links_removed'] = df['text'].apply(lambda x: re.split('https:\/\/.*', str(x))[0])
    ...
    df['tokenized'] = df['text_desc'].apply(word_tokenize)
...
df = preprocessing()
#df.to_csv('cleaned_data_test.csv')
